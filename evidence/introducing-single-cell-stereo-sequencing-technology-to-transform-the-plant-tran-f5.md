---
title: "Nevertheless, despite these limitations, technical improvements in Stereo-seq are expected to provid"
created: 2026-08-25
type: evidence
tags: [scrnaseq, plant-single-cell]
source: "[[introducing-single-cell-stereo-sequencing-technology-to-transform-the-plant-tran]]"
doi: "10.1016/j.tplants.2023.10.002"
species: [Arabidopsis thaliana, Oryza sativa, Zea mays]
evidence_type: "expression/regulation"
quality: "medium"
genes: []
compounds: []
---

# Nevertheless, despite these limitations, technical improvements in Stereo-seq are expected to provid

## Claim
Nevertheless, despite these limitations, technical improvements in Stereo-seq are expected to provide new insights into tissue composition and function by com- bining single cell transcriptomics with chromatin accessibility in the same biological sample.

## Biological Context
Introducing single cell stereo-sequencing technology to transform the plant transcriptome landscape

## Supporting Evidence
*Source: [[introducing-single-cell-stereo-sequencing-technology-to-transform-the-plant-tran]]*

## Evidence Quality
**Type**: expression/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

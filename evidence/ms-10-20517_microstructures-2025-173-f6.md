---
title: "Lysosome-targeting technologies such as LYTACs have broadened the target scope to include membrane a"
created: 2026-08-25
type: evidence
tags: [papers]
source: "[[ms-10-20517_microstructures-2025-173]]"
doi: "10.20517/microstructures.2025.173"
species: [Rosa spp.]
evidence_type: "expression/regulation"
quality: "medium"
genes: []
compounds: []
---

# Lysosome-targeting technologies such as LYTACs have broadened the target scope to include membrane a

## Claim
Lysosome-targeting technologies such as LYTACs have broadened the target scope to include membrane and secreted proteins; however, they face challenges related to complex molecular structures, limited tissue penetration, and non-specific receptor expression that can induce degradation in normal tissues[20,21].

## Biological Context
Nano-TPD for precision protein degradation via endogenous or exogenous triggered release

## Supporting Evidence
*Source: [[ms-10-20517_microstructures-2025-173]]*

## Evidence Quality
**Type**: expression/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

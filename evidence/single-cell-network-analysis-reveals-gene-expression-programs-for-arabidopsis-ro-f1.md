---
title: "By crossing nrl27-1 with the auxin reporter line DR5rev::GFP (Friml et al., 2003), we found that the"
created: 2026-08-24
type: evidence
tags: [scrnaseq, plant-single-cell]
source: "[[single-cell-network-analysis-reveals-gene-expression-programs-for-arabidopsis-ro]]"
doi: "10.1016/j.xplc.2024.100978"
species: [Medicago spp.]
evidence_type: "DNA binding/regulation"
quality: "medium"
genes: []
compounds: [auxin]
---

# By crossing nrl27-1 with the auxin reporter line DR5rev::GFP (Friml et al., 2003), we found that the

## Claim
By crossing nrl27-1 with the auxin reporter line DR5rev::GFP (Friml et al., 2003), we found that the auxin level of columella cells was signiﬁcantly lower in nrl27-1 than in WT plants (Figure 7G and 7H).

## Biological Context
Single-cell network analysis reveals gene expression programs for Arabidopsis root development and metabolism

## Supporting Evidence
*Source: [[single-cell-network-analysis-reveals-gene-expression-programs-for-arabidopsis-ro]]*

## Evidence Quality
**Type**: DNA binding/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

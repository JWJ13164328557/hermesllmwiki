---
title: "In conclusion, these results show that the SHR pathway regulates the expression of a large number of"
created: 2026-08-28
type: evidence
tags: [#developmental-biology, papers]
source: "[[ref-journal-pbio-0040143]]"
doi: "10.1371/journal.pbio.0040143"
species: [Nicotiana tabacum]
evidence_type: "expression/regulation"
quality: "medium"
genes: []
compounds: []
---

# In conclusion, these results show that the SHR pathway regulates the expression of a large number of

## Claim
In conclusion, these results show that the SHR pathway regulates the expression of a large number of transcription factors.

## Biological Context
Whole-genome analysis of the SHORT-ROOT developmental pathway in Arabidopsis.

## Supporting Evidence
*Source: [[ref-journal-pbio-0040143]]*

## Evidence Quality
**Type**: expression/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

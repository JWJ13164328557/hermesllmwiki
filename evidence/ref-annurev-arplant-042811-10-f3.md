---
title: "In summary, the OC-derived WUS protein must be present in both the OC and the stem cells to maintain"
created: 2026-08-25
type: evidence
tags: [#developmental-biology, papers]
source: "[[ref-annurev-arplant-042811-10]]"
doi: "10.1146/annurev-arplant-042811-105555"
species: [Arabidopsis thaliana, Oryza sativa, Zea mays]
evidence_type: "expression/regulation"
quality: "medium"
genes: []
compounds: []
---

# In summary, the OC-derived WUS protein must be present in both the OC and the stem cells to maintain

## Claim
In summary, the OC-derived WUS protein must be present in both the OC and the stem cells to maintain the stem cells in an undifferentiated state.

## Biological Context
植物干细胞壁龛。

## Supporting Evidence
*Source: [[ref-annurev-arplant-042811-10]]*

## Evidence Quality
**Type**: expression/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

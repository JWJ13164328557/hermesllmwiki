---
title: "From the ChIP-seq data, we found that genes involved in cell-cycle progression, such as CYCLIN D3;1 "
created: 2026-08-25
type: evidence
tags: [scrnaseq, plant-single-cell]
source: "[[a-guiding-role-of-the-arabidopsis-circadian-clock-in-cell-differentiation-reveal]]"
doi: "10.1016/j.celrep.2022.111059"
species: [Arabidopsis thaliana, Nicotiana tabacum, Citrus spp.]
evidence_type: "DNA binding/regulation"
quality: "medium"
genes: [ChIP]
compounds: []
---

# From the ChIP-seq data, we found that genes involved in cell-cycle progression, such as CYCLIN D3;1 

## Claim
From the ChIP-seq data, we found that genes involved in cell-cycle progression, such as CYCLIN D3;1 (CYCD3;1), RETINOBLAS- TOMA-RELATED (RBR), and E2FC, were signiﬁcantly enriched (Figures 4C and 4D; Table S4C).

## Biological Context
A guiding role of the Arabidopsis circadian clock in cell differentiation revealed by time-series single-cell RNA sequencing

## Supporting Evidence
*Source: [[a-guiding-role-of-the-arabidopsis-circadian-clock-in-cell-differentiation-reveal]]*

## Evidence Quality
**Type**: DNA binding/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

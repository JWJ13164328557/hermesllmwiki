---
title: "In agreement with these ﬁndings, we found that tomato COI1 physically interacts only with JAZ1/2/5/7"
created: 2026-08-25
type: evidence
tags: [#stress-immunity, papers]
source: "[[the-myc2pub22jaz4-module-plays-a-crucial]]"
doi: "10.1016/j.molp.2024.02.006"
species: [Oryza sativa, Solanum lycopersicum, Glycine max]
evidence_type: "protein interaction"
quality: "medium"
genes: []
compounds: []
---

# In agreement with these ﬁndings, we found that tomato COI1 physically interacts only with JAZ1/2/5/7

## Claim
In agreement with these ﬁndings, we found that tomato COI1 physically interacts only with JAZ1/2/5/7 (Figure 3), whereas PUB22 physically interacts with JAZ1/3/4/6 (Figure 2).

## Biological Context
The MYC2–PUB22–JAZ4 模块 plays a crucial 作用 in jasmonate signaling in tomato

## Supporting Evidence
*Source: [[the-myc2pub22jaz4-module-plays-a-crucial]]*

## Evidence Quality
**Type**: protein interaction
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

---
title: "We found 12 genes implicated in auxin-related processes that are repressed by SHR and none among the"
created: 2026-08-28
type: evidence
tags: [#developmental-biology, papers]
source: "[[ref-journal-pbio-0040143]]"
doi: "10.1371/journal.pbio.0040143"
species: [Nicotiana tabacum]
evidence_type: "expression/regulation"
quality: "medium"
genes: []
compounds: [auxin]
---

# We found 12 genes implicated in auxin-related processes that are repressed by SHR and none among the

## Claim
We found 12 genes implicated in auxin-related processes that are repressed by SHR and none among the indirect targets activated by SHR (Table S5).

## Biological Context
Whole-genome analysis of the SHORT-ROOT developmental pathway in Arabidopsis.

## Supporting Evidence
*Source: [[ref-journal-pbio-0040143]]*

## Evidence Quality
**Type**: expression/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

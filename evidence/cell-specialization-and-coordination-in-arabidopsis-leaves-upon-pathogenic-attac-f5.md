---
title: "For instance, we found in the C5 marker genes, CASPL4D2 and CASPL1D1, corroborating the recent liter"
created: 2026-08-24
type: evidence
tags: [scrnaseq, plant-single-cell]
source: "[[cell-specialization-and-coordination-in-arabidopsis-leaves-upon-pathogenic-attac]]"
doi: "10.1016/j.xplc.2023.100676"
species: [Solanum lycopersicum]
evidence_type: "expression/regulation"
quality: "medium"
genes: []
compounds: [lignin]
---

# For instance, we found in the C5 marker genes, CASPL4D2 and CASPL1D1, corroborating the recent liter

## Claim
For instance, we found in the C5 marker genes, CASPL4D2 and CASPL1D1, corroborating the recent literature showing that these two genes encode two essential enzymes for the organization of lignin polymers in response to Pst (Lee et al., 2019).

## Biological Context
Cell specialization and coordination in Arabidopsis leaves upon pathogenic attack revealed by scRNA-seq

## Supporting Evidence
*Source: [[cell-specialization-and-coordination-in-arabidopsis-leaves-upon-pathogenic-attac]]*

## Evidence Quality
**Type**: expression/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

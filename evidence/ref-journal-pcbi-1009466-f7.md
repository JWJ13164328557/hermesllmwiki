---
title: "In Fig 3D we show the inferred process for k = 1, 5, 10, 20 where we have taken π0 to be uniform on "
created: 2026-09-01
type: evidence
tags: [#single-cell-spatial, papers]
source: "[[ref-journal-pcbi-1009466]]"
doi: "10.1371/journal.pcbi.1009466"
species: [Arabidopsis thaliana, Ficus carica]
evidence_type: "expression/regulation"
quality: "medium"
genes: []
compounds: []
---

# In Fig 3D we show the inferred process for k = 1, 5, 10, 20 where we have taken π0 to be uniform on 

## Claim
In Fig 3D we show the inferred process for k = 1, 5, 10, 20 where we have taken π0 to be uniform on the source sites.

## Biological Context
Optimal transport analysis reveals trajectories in steady-state systems.

## Supporting Evidence
*Source: [[ref-journal-pcbi-1009466]]*

## Evidence Quality
**Type**: expression/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

---
title: "HOMEODOMAIN LEUCINE ZIPPER (HD-ZIP III) genes, whose protein products interact with the type-B RRs a"
created: 2026-08-26
type: evidence
tags: [#hormone-signaling, papers]
source: "[[10-1146-annurev-arplant-042817]]"
doi: "10.1146/annurev-arplant-042817"
species: [Oryza sativa, Zea mays, Solanum lycopersicum]
evidence_type: "protein interaction"
quality: "medium"
genes: []
compounds: []
---

# HOMEODOMAIN LEUCINE ZIPPER (HD-ZIP III) genes, whose protein products interact with the type-B RRs a

## Claim
HOMEODOMAIN LEUCINE ZIPPER (HD-ZIP III) genes, whose protein products interact with the type-B RRs and bind the same WUS promoter region (140).

## Biological Context
10.1146/annurev-arplant-042817

## Supporting Evidence
*Source: [[10-1146-annurev-arplant-042817]]*

## Evidence Quality
**Type**: protein interaction
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

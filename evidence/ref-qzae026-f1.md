---
title: "Chromatin immunoprecipitation coupled with high- throughput sequencing (ChIP-seq) and cleavage under"
created: 2026-08-24
type: evidence
tags: [#single-cell-spatial, papers]
source: "[[ref-qzae026]]"
doi: "10.1093/gpbjnl/qzae026"
species: [Arabidopsis thaliana, Oryza sativa, Zea mays]
evidence_type: "DNA binding/regulation"
quality: "medium"
genes: [ChIP]
compounds: []
---

# Chromatin immunoprecipitation coupled with high- throughput sequencing (ChIP-seq) and cleavage under

## Claim
Chromatin immunoprecipitation coupled with high- throughput sequencing (ChIP-seq) and cleavage under targets and tagmentation (CUT&Tag) methods are frequently used to locate genome-wide histone alterations [100,101].

## Biological Context
Opportunities and Challenges in Advancing Plant Research with Single-cell Omics.

## Supporting Evidence
*Source: [[ref-qzae026]]*

## Evidence Quality
**Type**: DNA binding/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

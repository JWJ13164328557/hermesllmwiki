---
title: "Validation of the obtained protoplast is required to determine the composition of retrieved cell typ"
created: 2026-08-26
type: evidence
tags: [#single-cell-spatial, papers]
source: "[[cr-annurev-arplant-0817]]"
doi: "10.1146/annurev-arplant-081720-010120"
species: [Arabidopsis thaliana, Oryza sativa, Zea mays]
evidence_type: "DNA binding/regulation"
quality: "medium"
genes: []
compounds: []
---

# Validation of the obtained protoplast is required to determine the composition of retrieved cell typ

## Claim
Validation of the obtained protoplast is required to determine the composition of retrieved cell types; this could be performed by quantitative real-time PCR analysis to determine the relative expression levels of specific reporter genes.

## Biological Context
Advances and Opportunities in Single-Cell Transcriptomics for Plant Research.

## Supporting Evidence
*Source: [[cr-annurev-arplant-0817]]*

## Evidence Quality
**Type**: DNA binding/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

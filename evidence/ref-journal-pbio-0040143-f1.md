---
title: "Of the 389 indirect targets repressed by SHR, we found 37 genes (9.5%) that encode proteins annotate"
created: 2026-08-30
type: evidence
tags: [#developmental-biology, papers]
source: "[[ref-journal-pbio-0040143]]"
doi: "10.1371/journal.pbio.0040143"
species: [Nicotiana tabacum]
evidence_type: "expression/regulation"
quality: "medium"
genes: []
compounds: []
---

# Of the 389 indirect targets repressed by SHR, we found 37 genes (9.5%) that encode proteins annotate

## Claim
Of the 389 indirect targets repressed by SHR, we found 37 genes (9.5%) that encode proteins annotated as kinases (Table S4).

## Biological Context
Whole-genome analysis of the SHORT-ROOT developmental pathway in Arabidopsis.

## Supporting Evidence
*Source: [[ref-journal-pbio-0040143]]*

## Evidence Quality
**Type**: expression/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-

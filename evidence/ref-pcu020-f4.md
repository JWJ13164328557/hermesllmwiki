---
title: "Here, we show that the gene responsible for the sgr6 mutant is At2g36810 that encodes the HEAT (Hunt"
created: 2026-08-30
type: evidence
tags: [#stress-immunity, papers]
source: "[[ref-pcu020]]"
doi: "10.1093/pcp/pcu020"
species: [Arabidopsis thaliana, Zea mays, Ficus carica]
evidence_type: "genetic perturbation"
quality: "high"
genes: []
compounds: []
---

# Here, we show that the gene responsible for the sgr6 mutant is At2g36810 that encodes the HEAT (Hunt

## Claim
Here, we show that the gene responsible for the sgr6 mutant is At2g36810 that encodes the HEAT (Huntingtin, Elongation factor 3, A-subunit of protein phos- phatase 2A and TOR1) protein with an unknown function.

## Biological Context
A unique HEAT repeat-containing protein SHOOT GRAVITROPISM6 is involved in vacuo

## Supporting Evidence
*Source: [[ref-pcu020]]*

## Evidence Quality
**Type**: genetic perturbation
**Level**: high

## Contradictory Evidence
_None identified_

## Open Questions
-

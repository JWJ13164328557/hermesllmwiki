---
title: "In addition, we found clusters annotated as senescent leaf cells, dividing cells, and stressed cells"
created: 2026-08-25
type: evidence
tags: [scrnaseq, plant-single-cell]
source: "[[shoot-and-root-single-cell-sequencing-reveals-tissue-and-daytime-specific-trans]]"
doi: "10.1093/plphys/kiab537"
species: [Arabidopsis thaliana]
evidence_type: "expression/regulation"
quality: "medium"
genes: []
compounds: []
---

# In addition, we found clusters annotated as senescent leaf cells, dividing cells, and stressed cells

## Claim
In addition, we found clusters annotated as senescent leaf cells, dividing cells, and stressed cells (for detailed information, see Supplemental Text S4 and Supplemental Figure S8).

## Biological Context
Shoot and root single cell sequencing reveals tissue- and daytime-specific transcriptome profiles

## Supporting Evidence
*Source: [[shoot-and-root-single-cell-sequencing-reveals-tissue-and-daytime-specific-trans]]*

## Evidence Quality
**Type**: expression/regulation
**Level**: medium

## Contradictory Evidence
_None identified_

## Open Questions
-
